...
import line.tracker.LineTracker;
...
public class DrawOnTop extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (mBitmap != null)
        {
        	...
        	canvas.drawText("Orientation - Azimuth: "+LineTracker.azimuth, 200, -80, mPaint);
        	canvas.drawText("Orientation - Pitch: "+LineTracker.pitch, 200, -50, mPaint);
        	canvas.drawText("Orientation - Roll: "+LineTracker.roll, 200, -20, mPaint);
        	
        	if (tracking == 0) {
            	mPaint.setTextSize(40);
            	mPaint.setStyle(Paint.Style.FILL);
        		Rect rect = new Rect();
        		mPaint.getTextBounds("PAUSED", 0, "PAUSED".length(), rect);
        		canvas.drawText("PAUSED", canvasWidth/2-rect.exactCenterX(), canvasHeight/2, mPaint);  
        	} else {
